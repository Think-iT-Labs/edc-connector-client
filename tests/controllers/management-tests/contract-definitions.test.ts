  ...
  (apiVersion) => {
    ...
    describe("delete", () => {
      it("deletes a target contract definition", async () => {
        ...
        const contractDefinition = await contractDefinitions.delete(
          idResponse.id,
        );
      ...
      describe("update", () => {
        it("updates a target contract definition", async () => {
          ...
          await contractDefinitions.update(updateContractDefinitionInput);

          const updatedContractDefinition = await contractDefinitions.get(
            updateContractDefinitionInput["@id"]!,
          );

          expect(updatedContractDefinition).toHaveProperty("@type");
          expect(updatedContractDefinition.accessPolicyId).toEqual(
            updateContractDefinitionInput.accessPolicyId,
          );
          expect(updatedContractDefinition.contractPolicyId).toEqual(
            updateContractDefinitionInput.contractPolicyId,
          );
          expect(updatedContractDefinition.assetsSelector).toEqual(
            updateContractDefinitionInput.assetsSelector,
          );
        });
        ...
        it("fails to update an inexistant contract definition", async () => {
          ...
          const maybeUpdatedContractDefinition = contractDefinitions.update(
