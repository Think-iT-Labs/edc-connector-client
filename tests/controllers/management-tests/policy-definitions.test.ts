    ...
    describe("get", () => {
      it("succesfully return a policy definition", async () => {
        ...
        const prohibitionsConstraints =
          policyDefinition.policy.prohibitions[0].array("odrl", "constraint");
        expect(prohibitionsConstraints).toHaveLength(1);
        expect(policyDefinition.policy.obligations).toHaveLength(1);
        const obligationsConstraints =
          policyDefinition.policy.obligations[0].array("odrl", "constraint");
